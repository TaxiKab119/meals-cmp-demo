...
class MealsDbClient(
    ...
    suspend fun getMealDetails(mealId: String) : Result<MealDetails, NetworkError> {
        val response = try {
            httpClient.get(urlString = "https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealId}")
        } catch (e: UnresolvedAddressException) {
            return Result.Error(NetworkError.NO_INTERNET)
        } catch (e: SerializationException) {
            return Result.Error(NetworkError.SERIALIZATION)
        ...
        return when (response.status.value) {
            ...
            in 200..299 -> {
                try {
                    val mealDetails = response.body<MealDetailsResponse>()
                    Result.Success(mealDetails.mealDetails[0])
                } catch (e: SerializationException) {
                    Result.Error(NetworkError.SERIALIZATION)
                }
            }
